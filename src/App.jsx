...
import { 
  ...
  Building2, ChevronRight, Save, UserCog, User, Briefcase
} from 'lucide-react';
import { 
  PieChart, Pie, Cell, ResponsiveContainer, Tooltip, Legend 
} from 'recharts';

// --- CONFIGURAÇÃO DOS CRITÉRIOS DE AVALIAÇÃO (Regra de Negócio) ---

const CRITERIA_RULES = {
  // Cenário 1: Supervisor avalia Liderado
  'sup_to_liderado': [
  ...
  // Cenário 2: Liderado avalia Supervisor
  'liderado_to_sup': [
  ...
  // Cenário 3: Supervisor avalia Coord. Terra
  'sup_to_coord': [
    { id: 'esc', label: 'Esclarecimento sobre o Trabalho', peso: 0.30 },
    { id: 'tec', label: 'Conhecimento Técnico', peso: 0.30 },
    { id: 'sup', label: 'Suporte e Comunicação', peso: 0.25 },
    { id: 'rel', label: 'Relacionamento Interpessoal', peso: 0.15 },
  ],
  // Cenário 4: Coord. Terra avalia Supervisor
  'coord_to_sup': [
    { id: 'tec', label: 'Qualidade Técnica', peso: 0.25 },
    { id: 'sms', label: 'Atenção às Exigências de SMS', peso: 0.25 },
    { id: 'cli', label: 'Relacionamento com Cliente', peso: 0.25 },
    { id: 'lid', label: 'Liderança', peso: 0.25 },
  ],
  // Cenário 5: Base (Etapa 2)
  'base_depts': [
...
// --- DADOS MOCKADOS (Pendências de Avaliação conforme o Logado) ---

const PENDING_EVALUATIONS = {
  'supervisor': [
    { id: '1', name: 'João Silva (Soldador)', type: 'liderado', context: 'sup_to_liderado' },
    { id: '2', name: 'Pedro Santos (Eletricista)', type: 'liderado', context: 'sup_to_liderado' },
  ...
  'gestor_base': [
    { id: '1', name: 'João Silva', type: 'colaborador', context: 'base_depts' },
    { id: '2', name: 'Pedro Santos', type: 'colaborador', context: 'base_depts' }
  ]
};
...
const EvaluationForm = ({ target, onBack }) => {
  ...
  const criteriaList = CRITERIA_RULES[target.context];
    ...
    <div className="bg-white rounded-xl shadow-lg border border-slate-200 overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="bg-slate-50 p-6 border-b border-slate-200 flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-slate-500">Avaliando: <span className="font-bold text-slate-700">{target.name}</span></p>
        </div>
      ...
      <div className="p-6 grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="bg-slate-50 p-6 rounded-xl h-fit border border-slate-200 sticky top-6">
          ...
          <div className="mt-2 text-center text-xs text-slate-400">{status.label}</div>
    ...
    return (
      <div className="min-h-screen bg-slate-900 flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl p-8 max-w-2xl w-full shadow-2xl animate-in zoom-in duration-300">
          ...
          <p className="text-center text-xs text-slate-400 mt-8">© 2025 Vertical Group Engineering. v3.0 Alpha</p>
        </div>
      </div>
    );
  }
